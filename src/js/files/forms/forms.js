...
export let formValidate = {
  ...
  formClean(form) {
    ...
    setTimeout(() => {
      ...
      for (let index = 0; index < inputs.length; index++) {
        ...
        el.classList.remove("_form-field");
        el.parentElement.classList.remove("_form-field");
  ...
  //   let isValid = phoneNumberPattern.test(formRequiredItem.value);
  ...
  async function formSubmitAction(form, e) {
    ...
    if (error === 0) {
      ...
      if (ajax) {
        ...
        const response = await fetch(formAction, data);
        if (response.ok) {
...
const removeSended = document.querySelectorAll(".js-remove-sended");
if (removeSended[0]) {
  removeSended.forEach((element) => {
    element.addEventListener("click", (e) => {
      e.preventDefault();
      const form = e.target.closest("._sended");
      if (form) {
...
//       // Проверка на существование phoneInput и свойства inputmask
//       let phoneInput = form.querySelector(".form__input-phone");
//       if (
//         phoneInput &&
//         phoneInput.inputmask &&
//         typeof phoneInput.inputmask.isComplete === "function" &&
